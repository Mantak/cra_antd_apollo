...
import SIGN_UP from '../../graphql/users/sign_up_mutation.gql';
    ...
    this.props.form.validateFields({ force: true }, (err, values) => {
      if (!err) {
        ...
        return signUp(values).then(response => {
          ...
          if (!payload.errors) {
            localForage.setItem(tokenName, payload.currentUser.token).then(() => {
              this.props.history.push('/');
            });
          } else {
            this.setState({ loading: false });
            this.props.form.setFieldsValue({ password: '', passwordConfirmation: '' });
          }
        });
      }
    });
